  ...
  <main class="enterprise-auth">
    ...
    <div class="stage">
      <section class="auth-card">
        <p v-if="brand.title || brand.body" class="brand-intro">{{ brand.title }}<template v-if="brand.title && brand.body"> · </template>{{ brand.body }}</p>
        <slot />
      </section>
    </div>
    <footer class="auth-footer">仅限授权企业成员访问 · 不会在页面显示密码、Token 或完整 API Key</footer>
  </main>
</template>
