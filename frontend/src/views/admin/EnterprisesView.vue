<template>
  <PlatformEnterpriseShell><section><div class="heading"><div><h1>企业租户</h1><p>创建和管理企业入口、专用上游用户及租户状态。</p></div><el-button type="primary" :icon="Plus" @click="router.push('/admin/enterprises/new')">开通企业</el-button></div><div class="filters"><el-input v-model="search" clearable placeholder="搜索名称或入口域名" @keyup.enter="load"/><el-select v-model="status" clearable placeholder="全部状态" @change="load"><el-option label="启用" value="active"/><el-option label="停用" value="disabled"/></el-select><el-button :icon="Search" @click="load">查询</el-button></div><el-alert v-if="!loading&&sourceUnavailable" title="企业列表来源不可用" description="当前未取得平台实时数据，以下不展示旧数据；请刷新后重试。" type="error" show-icon class="source-alert" data-testid="enterprises-source-unavailable"/><el-table v-else v-loading="loading" :data="items"><el-table-column prop="name" label="企业" min-width="180"/><el-table-column prop="portal_host" label="入口域名" min-width="220"/><el-table-column prop="dedicated_upstream_user_id" label="专用用户 ID" width="130"/><el-table-column prop="admin_email" label="主账号" width="180"/><el-table-column prop="status" label="状态" width="100"/><el-table-column label="操作" width="180"><template #default="{row}"><el-button text @click="router.push(`/admin/enterprises/${row.id}`)">查看</el-button><el-button v-if="row.status==='active'" text type="danger" @click="disable(row)">停用</el-button></template></el-table-column></el-table><el-empty v-if="!loading&&!sourceUnavailable&&!items.length" description="暂无企业租户"/>
...
<style scoped>.heading{display:flex;justify-content:space-between;gap:20px;margin-bottom:20px}.heading h1{margin:0 0 6px;font-size:26px}.heading p{color:#64748b}.filters{display:flex;gap:12px;margin-bottom:18px}.filters .el-input{max-width:340px}.filters .el-select{width:160px}.source-alert{margin-bottom:12px}@media(max-width:640px){.heading,.filters{flex-direction:column}.filters .el-input,.filters .el-select{width:100%;max-width:none}}</style>
