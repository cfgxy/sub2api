...
describe('Stripe lazy-loading contract', () => {
  ...
  it('keeps Stripe out of automatic vendor chunking', () => {
    const viteConfig = readFrontendFile('vite.config.ts')
    const stripeRule = viteConfig.indexOf("id.includes('/@stripe/stripe-js/')")
    const automaticChunkingFallback = viteConfig.indexOf('return undefined')

    expect(stripeRule).toBeGreaterThan(-1)
    expect(viteConfig.slice(stripeRule, automaticChunkingFallback)).toContain("return 'vendor-stripe'")
    expect(stripeRule).toBeLessThan(automaticChunkingFallback)
  })
})
